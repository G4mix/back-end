...
describe('Get Followers Integration Tests', () => {
	...
	describe('GET /api/v1/follow/followers/:userId', () => {
		it('should get followers with pagination successfully', async () => {
			...
			// Act
			const response = await httpClient.get(`/api/v1/follow/followers/${userId}`, {
				page: 0,
				limit: 10
			})
		...
		it('should return validation error for invalid user ID', async () => {
			// Act & Assert
			await expect(httpClient.get('/api/v1/follow/followers/invalid-uuid'))
		...
		it('should return validation error for invalid page number', async () => {
			...
			// Act & Assert
			await expect(httpClient.get(`/api/v1/follow/followers/${userId}`, { page: -1 }))
				.rejects.toMatchObject({
					response: {
						status: 400,
						data: {
		...
		it('should return validation error for invalid limit', async () => {
			...
			// Act & Assert
			await expect(httpClient.get(`/api/v1/follow/followers/${userId}`, { limit: 200 }))
				.rejects.toMatchObject({
					response: {
						status: 400,
						data: {
		...
		it('should handle database errors gracefully', async () => {
			...
			// Act & Assert
			await expect(httpClient.get(`/api/v1/follow/followers/${userId}`, { page: 0, limit: 10 }))
				.rejects.toMatchObject({
					response: {
						status: 500
					}
